...
/*   Updated: 2025/03/15 17:25:46 by ychedmi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_other(t_list *map)
{
	...
	while (map)
	{
		...
		while (map->data[i])
		{
			if (map->data[i] != '1' && map->data[i] != '0' \
			&& map->data[i] != 'C' \
			&& map->data[i] != 'E' && map->data[i] != 'P' \
			&& map->data[i] != '\n')
...
int	checkdb(t_list *map)
{
	...
	{
		write(2, "Error\n", 7);
		write(2, "Player Not Valid !\n", 20);
		return (1);
	}
	if (countpec(map, 'E') != 1)
	{
		write(2, "Error\n", 7);
		write(2, "Exit Not Valid !\n", 18);
		return (1);
	}
	if (countpec(map, 'C') < 1)
	{
		write(2, "Error\n", 7);
		write(2, "Collectable Not Valid !\n", 25);
		return (1);
	}
	return (0);
}
